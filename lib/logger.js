  ...
  async onrequest(ctx, next) {
    ...
    try {
      ...
      if (!ctx.skipLog) {
        ...
        const publicKey = idEnc.toString(ctx.connection.remotePublicKey)
        const ipPrefix = this._logIp
          ? `[ip=${ctx.connection?.rawStream?.remoteHost ?? 'unknown'}] `
          : ''
        if (caughtError) {
          this._logger.warn(
            `${ipPrefix}[method=${ctx.method}] [publicKey=${publicKey}] failed with [message=${caughtError.message}] [code=${caughtError.code}] after ${durationMs}ms`
          )
        } else {
          this._logger.info(
            `${ipPrefix}[method=${ctx.method}] [publicKey=${publicKey}] succeeded after ${durationMs}ms`
          )
        }
      }
    }
