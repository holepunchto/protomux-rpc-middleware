  ...
  async onrequest(ctx, next) {
    ...
    try {
      ...
      if (!ctx.skipLog) {
        ...
        if (caughtError) {
          this._logger.warn(
            `${ipPrefix}[requestId=${ctx.requestId}] [method=${ctx.method}] [publicKey=${publicKey}] failed with [message=${caughtError.message}] [code=${caughtError.code}] [stack=${caughtError.stack}] after ${durationMs}ms`
          )
        } else {
          this._logger.info(
